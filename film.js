...
if (filmDetail) {
	// nazev filmu
	detail.querySelector('.card-title').textContent= filmDetail.nazev;
	// dlouhej popis
...
if (!messageInput.value.trim()) {
	message.messageInput.classList.add("is-invalid");
		messageInput.focus();
		valid= false;
}

// souhlasil uzivatel s podminkami?
...
let textRozdil;
if (rozdilDni > 0) {
	textRozdil = `, což bude za ${rozdilDni} dní.`;
} else if (rozdilDni < 0) {
	textRozdil = `, což bylo před ${Math.abs(rozdilDni)} dny.`;
} else {
	textRozdil = `, což je dnes.`;
}
premiElement.innerHTML = `Premiéra <strong>${datumFormatted}</strong>${textRozdil}`;
